...
object Par {
  ...
  def choiceByN[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] =
  ...
  def choicNByGen[A](n: Par[Int])(choices:  List[Par[A]]): Par[A] =
    flatMap(n)(choices)

  def choiceMap[K,V](key: Par[K])(choices: Map[K, Par[V]]): Par[V] =
    flatMap(key)(choices)

  def flatMap[A,B](n: Par[A])(f: A => Par[B]): Par[B] =
    es => {
      val ind = run(es)(n).get // Full source files
      run(es)(f(ind))
    }
  ...
  def parFilter[A](l: List[A])(f: A => Boolean): Par[List[A]] = {
    val pars: List[Par[List[A]]] =
      l map (asynF((a: A) => if (f(a)) List(a) else List()))
